...
bool check_vec_size ( const char* func_name, const char* vec_name,
                      const bmp_io_vec_type &vec, const size_t expected_size )
{
  string func_name_in_caps = func_name;
  transform(func_name_in_caps.begin(), func_name_in_caps.end(), func_name_in_caps.begin(), ::toupper);

  bool error = (vec.size() != expected_size);

  if ( error ) {
    cout << "\n";
    cout << func_name_in_caps << ": Fatal error!\n";
    cout << "  Size of " << vec_name << " does not match expected size.\n";
    cout << "  Expected size = " << expected_size << "\n";
    cout << "  Actual size = " << vec.size() << "\n";
    return error;
  }
...
//
  ...
  for ( j = 0; j < abs ( height ); j++ ) {
    for ( i2 = 0; i2 < width; i2++ ) {
      ...
      rvec.push_back(( unsigned char ) c);
      numbyte = numbyte + 1;
    }
//
//  If necessary, read a few padding characters.
...
bool bmp_08_data_write ( ofstream &file_out, unsigned long int width, 
                         long int height, const bmp_io_vec_type &rvec)
{
  size_t expected_size = width*height;
  bool error = check_vec_size("bmp_08_data_write", "rvec", rvec, expected_size);
...
bool bmp_24_data_read ( ifstream &file_in, unsigned long int width, 
                        long int height, bmp_io_vec_type &rvec, bmp_io_vec_type &gvec,
                        bmp_io_vec_type &bvec )
{
  char c;
  bool error;
  int i;
...
//
  ...
  for ( j = 0; j < abs ( height ); j++ ) {
    for ( i2 = 0; i2 < width; i2++ ) {
      ...
      bvec.push_back(( unsigned char ) c);
      numbyte = numbyte + 1;
      ...
      gvec.push_back(( unsigned char ) c);
      numbyte = numbyte + 1;
...
bool bmp_24_data_write ( ofstream &file_out, unsigned long int width, 
                         long int height, bmp_io_vec_type &rvec, bmp_io_vec_type &gvec,
                         bmp_io_vec_type &bvec )
{
  size_t expected_size = width*height;
  bool error;
  error = check_vec_size("bmp_24_data_write", "rvec", rvec, expected_size);
  error = check_vec_size("bmp_24_data_write", "gvec", gvec, expected_size) || error;
  error = check_vec_size("bmp_24_data_write", "bvec", bvec, expected_size) || error;
  if (error) { return true; }
...
//
  ...
  for ( j = 0; j < abs ( height ); j++ ) {
    for ( i2 = 0; i2 < width; i2++ ) {
      file_out << bvec.at(index);
      file_out << gvec.at(index);
      file_out << rvec.at(index);
...
{
  size_t expected_size = colorsused;
  bool error;
  error = check_vec_size("bmp_palette_print", "rpvec", rpvec, expected_size);
  error = check_vec_size("bmp_palette_print", "gpvec", gpvec, expected_size) || error;
  error = check_vec_size("bmp_palette_print", "bpvec", bpvec, expected_size) || error;
  error = check_vec_size("bmp_palette_print", "apvec", apvec, expected_size) || error;
  if (error) { return true; }
  ...
  for ( i = 0; i < colorsused; i++ ) {
    cout << setw(6) << i       << "  "
         << setw(6) << bpvec.at(index) << "  "
         << setw(6) << gpvec.at(index) << "  "
         << setw(6) << rpvec.at(index) << "  "
         << setw(6) << apvec.at(index) << "\n";
  ...
  bmp_io_vec_type &apvec )
{
  ...
  for ( i = 0; i < colorsused; i++ ) {
    ...
    bpvec.push_back(( unsigned char ) c);
    ...
    gpvec.push_back(( unsigned char ) c);
    ...
    rpvec.push_back(( unsigned char ) c);
    ...
    apvec.push_back(( unsigned char ) c);
  }
...
bool bmp_palette_write ( ofstream &file_out, unsigned long int colorsused, 
  bmp_io_vec_type &rpvec, bmp_io_vec_type &gpvec, bmp_io_vec_type &bpvec,
  bmp_io_vec_type &apvec )
{
  size_t expected_size = colorsused;
  bool error;
  error = check_vec_size("bmp_palette_write", "rpvec", rpvec, expected_size);
  error = check_vec_size("bmp_palette_write", "gpvec", gpvec, expected_size) || error;
  error = check_vec_size("bmp_palette_write", "bpvec", bpvec, expected_size) || error;
  error = check_vec_size("bmp_palette_write", "apvec", apvec, expected_size) || error;
  if (error) { return true; }
  ...
  for ( i = 0; i < colorsused; i++ ) {
    file_out << bpvec.at(index);
    file_out << gpvec.at(index);
    file_out << rpvec.at(index);
    file_out << apvec.at(index);
...
//
  if ( bitsperpixel == 8 ) {
    ...
    // Doing a deep copy of the arrays is more memory safe and also avoids compiler warnings.
...
//  Purpose:
//
//    BMP_GET_DIMS gets dimensions of an input image. These dimensions can be
//    used to allocate storage for the pixel arrays externally.
//
//****************************************************************************80
...
//  Open the input file.
//
  ifstream file_in ( file_in_name, ios::in | ios::binary );

  bool error;
  if ( !file_in ) {
    error = true;
    cout << "\n";
    cout << "BMP_GET_DIMS - Fatal error!\n";
    cout << "  Could not open the input file.\n";
    return error;
  }
//
...
// If internal_alloc = true, rarray, garray and barray are internally allocated.
...
// Internally allocate the arrays if internal_alloc = true.
  if ( internal_alloc ) {
    *rarray = new unsigned char[numbytes];
    *garray = new unsigned char[numbytes];
    *barray = new unsigned char[numbytes];
                ...
                bmp_io_vec_type &rvec, bmp_io_vec_type &gvec, bmp_io_vec_type &bvec )
{
  ...
  unsigned long int filesize;
  unsigned short int filetype;
  bmp_io_vec_type gpvec;
  unsigned long int horzresolution;
  unsigned short int magic;
  unsigned short int planes;
  unsigned short int reserved1;
  unsigned short int reserved2;
  bmp_io_vec_type rpvec;
  unsigned long int size;
  unsigned long int sizeofbitmap;
  unsigned long int vertresolution;
//
...
//
  if ( 0 < colorsused ) {
    error = bmp_palette_read ( file_in, colorsused, rpvec, gpvec,
                               bpvec, apvec );
...
//
  ...
  if ( bitsperpixel == 8 ) {
    ...
    error = bmp_08_data_read ( file_in, *width, *height, rvec );
    ...
    error = bmp_24_data_read ( file_in, *width, *height, rvec, gvec, bvec );
...
//  Write the palette.
//
  error = bmp_palette_write ( file_out, colorsused, rpvec, gpvec, bpvec, apvec );
  if ( error ) {
    return error;
  }
//
//  Write the data.
//
  error = bmp_08_data_write ( file_out, width, height, rvec );
  if ( error ) {
    return error;
  }
//
...
//  Write the data.
//
  error = bmp_24_data_write ( file_out, width, height, rvec, gvec, bvec );
  if (error) {
    return error;
  }
//
